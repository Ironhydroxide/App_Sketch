...
void BootupConfig()
{
	Gpu_Hal_Powercycle(phost,1);//cycles PD pin (low/high for 1, high/low for FALSE)
...
#if (/*defined(800_ENABLE) || *//*defined(810_ENABLE) ||*/ defined(T812_ENABLE))
...
// Touch Screen Calibration
  ...
  App_WrCoCmd_Buffer(phost,CLEAR(1,1,1));
  App_WrCoCmd_Buffer(phost,COLOR_RGB(255,255,255));
  Serial.println("Show tap on dot");
  delay(1000);
...
// Ftdi Logo animation 
  ...
  do
  {
   ...
   // INFORMATION 
	...
	Serial.println("show click to play");
...
/*#ifdef FT900_PLATFORM
	FT900_Config();
#endif*/
	Serial.println("Starting, line 769");
	delay(1000);
...
/*#ifdef MSVC_PLATFORM_SPI
  ...
  host.hal_config.spi_clockrate_khz = 29000; //in KHz//30000kHz is max for FT813
#endif
	Serial.println("Starting Screen, PD pin high and SPI settings .");
  delay(1000);
  Gpu_Hal_Open(&host);//pulls PD pin high, Pulls CS pin High, Starts SPI (badly), SPI Settings

  phost = &host;
	Serial.println("Initialize screen, cycle PD pin, send configs to screen");
    delay(1000);
...
Serial.println("Calling HomeSetup");
  delay(1000);home_setup();
